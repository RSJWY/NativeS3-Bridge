...
#### Node storage telemetry (heartbeat)

- `HeartbeatPayload` telemetry is a single optional group: `used_bytes_total`/`object_count` are pointers and `observed_at` is an RFC3339 UTC string. Field absence (legacy node) and a legitimate `0` are different states; parse via `HeartbeatPayload.Telemetry()`, which returns `ok=false` unless all three fields are complete and well-formed. Partial or malformed groups are "unavailable", never a zero snapshot.
- The node is the only collector. The heartbeat reads the singleton `nodeagent.StorageTelemetry` row (single-row constant-cost query; the snapshot builder takes only the DB handle so a filesystem walk is structurally impossible). Panel never pulls or scans node disks.
- Node counters are independent of credential quota accounting (`credentials.used_bytes` is a quota view and can be distorted). The baseline is one synchronous full scan of the data root before the S3 listener accepts traffic, reusing `storage.ReconcileBucket` exclusion rules via `storage.ScanDataRoot`; the explicit `storage_reconcile_apply` task is the only repair path and rebuilds the counters after apply.
- Recorder rules: mutations after the native write commits (`bytes += newSize-oldSize`; object count from existence, not size). Invalid counters are never revived by increments - only a baseline rebuild sets `Valid=true`. A failed accounting update marks telemetry invalid and logs; it must never fail the S3 request nor report a fabricated value.
- Panel `NodeState` telemetry columns are nullable (NULL = never reported) plus `telemetry_valid`. A legacy heartbeat sets `telemetry_valid=false` while keeping old values inspectable; it never writes zeros. Summary aggregation counts only valid-and-fresh snapshots, classifying nodes as `valid`/`missing`/`stale` with the same threshold source as offline detection (`heartbeat_interval * offline_multiplier` injected into `AdminAPI`), never a second hardcoded default.
...
- Push whose published snapshot fails integrity verification (`ErrDesiredSnapshotHashMismatch`) -> HTTP `409` with the republish hint, not `500`. Hash mismatch means "the admin must publish again", which is a conflict, not a server fault. Same for missing capability and legacy snapshot.
- Any node-scoped subroute that reloads the node after a write must honour the `loadNode` ok flag before writing a success body. Ignoring it emits a second response after the `404`/`500` already went out (and ships a zero-valued node object).
- Node-scoped subroutes with no deeper resource (`tokens`, `certs/revoke`) must validate the path segment count; extra segments are `404`, never a silently accepted request.
- Retired nodes reject writes on `credentials` / `buckets` / `webhooks` / `rate-limit` with HTTP `409`, but `GET` stays `200`. Retirement is irreversible and the node row is deliberately retained for audit, so read visibility must survive; only new drafts (which could never reach the node) are refused.

### 4b. Control-Plane Robustness Contracts

These hold for the node control plane (`pkg/panel/transport.go`, `agentconn.go`, `tasks.go`) and exist because a single misbehaving node must not degrade the panel for every other node.

- **Writes are interruptible.** Serialize control-plane writes with a ctx-aware semaphore, never `sync.Mutex`: a mutex wait ignores ctx, so a peer that stops reading wedges every later writer forever while the write timeout only covers the `ws.Write` after the lock. Every write call site (heartbeat ack, task dispatch, desired-state push, import request) must pass a ctx with `writeTimeout`, never the bare serve ctx (which cancels only at connection close).
- **Task state moves forward only.** `markState` guards on `state = 'pending'`. A node's result can land before the dispatcher's `markState(running)`; without the guard the late transition overwrites the terminal state and result. An empty error string means "nothing to record", not "blank the stored failure reason". Guard misses log at Info — that is the normal outcome of the race, not an error.
- **Heartbeat persistence is throttled, liveness is not.** Update in-memory state and ack on every frame, but throttle the DB upsert to `heartbeat_interval / 2`, derived from the configured cadence rather than a constant. This keeps per-frame persistence identical at normal cadence while bounding flood writes to `2/interval`. The safety invariant that lets `SweepOffline` keep reading DB `last_heartbeat`: persistence lags at most `interval/2` while the offline threshold is `offline_multiplier * interval` — under a sixth of it. Advance the throttle timestamp only **after** a successful write, otherwise one failed upsert also eats the next retry window.
- **Storage errors are graded; protocol errors are not.** Transient storage failures (heartbeat/ack/task-result persistence, desired-config lookups) are absorbed inside the handler: log, count, return nil. Only after `maxConsecutiveStorageFailures` consecutive failures does a sentinel error close the connection. Protocol errors (undecodable payload, invalid sync state, unknown message type) keep dropping the connection immediately. Context cancellation is never counted — that is a normal close, not a storage fault. Any successful write resets the counter.
- **Certificate renewal is rate limited.** `/renew` is capped per node identity (`maxRenewPerWindow` per `renewWindow`), rejected with `429` + `Retry-After` **before** CSR parsing, and audited as `rate_limited`. Each renewal costs a CA signature plus a `node_certs` row, so an unbounded endpoint is a free CPU and table-growth vector. Rejected attempts must not be recorded, or repeated retries push the window forward indefinitely.
- **Node-reported free text is sanitized on every write path.** `region` and `content_hash` arrive as node-controlled strings and reach admin pages. Strip control characters, trim, and truncate **on rune boundaries** — byte slicing splits multi-byte characters, and the resulting invalid UTF-8 is rejected by MySQL/Postgres `utf8mb4` columns, which then trips the storage-failure counter and disconnects the node. Sanitize at *every* site that writes the column (both the hello and the ack path write `content_hash`); compare against published values using the raw string.
- **Protocol version is bumped to v2 with synchronous upgrade semantics.** `ProtocolVersion` and `MinCompatibleVersion` are both `2`; v1 peers are rejected at the hello handshake with a fatal `version_incompatible` error that names both sides' versions and tells operators to synchronize upgrades. Node reconnects with backoff; it does not downgrade or silently skip the error.
- **Heartbeat cadence is negotiated, not assumed.** The node reports `heartbeat_interval_ms` in hello. Panel clamps it to `[1s, 10min]` and falls back to the panel-configured interval on out-of-bounds values (logging a warning). The clamped value drives both the offline threshold (`heartbeat_interval * offline_multiplier`) and the per-connection read timeout (`threshold + 30s` margin). This prevents a misbehaving node from pushing its offline detection to infinity and prevents a fixed read timeout from killing long-heartbeat nodes.
- **Control-plane reads have a bounded lifetime.** Panel's serve loop wraps each `readEnvelope` in a `context.WithTimeout` derived from the negotiated heartbeat interval. A silent connection is closed when the read timeout fires; the connection cleanup only unregisters the socket — `SweepOffline` remains the sole writer of the DB `online` column.
- **Task timeout is enforced on the node.** The panel sends `timeout_ms` on every task frame, sourced from the panel-level `task_timeout` config (default `60s`). The node wraps task execution in `context.WithTimeout`; it clamps the value to a hard-coded 10-minute maximum and falls back to 60s for non-positive values. A timed-out task returns `state=failed,error=timeout` and is **not** recorded in the idempotency ledger (only successful results are cached).
- **Import reports are paginated and reassembled with bounded memory.** v2 sends `import_report_chunk` frames of at most 512 KiB each. Panel reassembles them per `request_id` on the connection; the reassembly buffer is capped at 32 chunks / 16 MiB / 5 minutes, after which the chunk is dropped and the connection is closed. This prevents a malicious or stuck node from pinning unbounded panel memory with incomplete reports.
- **Node certificates are validated before persistence.** The node validates every panel-issued certificate (registration and renewal) before writing it: valid PEM X.509, public key matches the local private key, chain verifies against the panel CA, and `NotAfter` is in the future and within a 10-year ceiling. Any failure leaves the existing cert file untouched and retries/alarms through the normal path.
- **PEM files are written atomically.** Node identity files are written as same-directory temp files, `fsync`'d, and renamed into place. Existing files are backed up as `.bak` before overwrite. This eliminates torn writes from crashes during cert/key updates.
- **The node serve loop has a read watchdog.** A goroutine monitors the last successful frame read time; if no frame arrives within `max(3 * heartbeat_interval, 60s)`, it closes the connection and reconnects. The timer resets on every received frame, not just heartbeat acks.
- **Heartbeat send failures close the connection.** If a periodic heartbeat write fails, the node closes the WebSocket immediately so the serve loop exits and reconnects, rather than leaving a half-open connection idle.
- **Malformed tasks are rejected before the ledger.** The node rejects tasks with empty `task_id` or unknown `type` at the entry of `handleTask`; they are not executed and are not recorded in the idempotency ledger. The idempotency cache also checks that the cached `type` matches the incoming `type`.
- **Per-frame version is validated after handshake.** The node records the negotiated protocol version from `hello_ack` and drops any later frame whose `version` exceeds it, logging a warning. The connection stays open; only the malformed/higher-version frame is ignored.
- **`node --health` probes the S3 listener specifically.** The probe sends `GET /` and requires a 403 response containing a valid S3 XML `<Error><Code>...</Code></Error>` body. It rejects plain HTTP services and only skips TLS verification for loopback addresses.
...
- Do not turn node telemetry absence into zero. Non-pointer payload fields, `not null default 0` Panel columns, or summing missing nodes into totals all fabricate data; use pointers/nullable columns and exclude `missing`/`stale` nodes from aggregates.
- Do not scan the storage root on the heartbeat path, and do not let incremental telemetry deltas re-validate an invalid counter; only the startup baseline or explicit reconcile rebuild may restore `Valid`.
- Do not serialize control-plane writes with `sync.Mutex`, and do not hand a write the bare serve ctx. Both make "peer stopped reading" unrecoverable: the mutex wait ignores ctx, and the serve ctx only cancels at connection close.
- Do not close a control connection because one DB write failed. A transient failure drops one frame; dropping the connection also marks every in-flight task on it as interrupted. Grade the error and only disconnect on sustained failure.
- Do not count context cancellation as a storage failure, and do not let a failed write advance the heartbeat throttle timestamp — the first pollutes the next connection's budget, the second shrinks the retry window exactly when the DB is struggling.
- Do not truncate node-reported text by byte slicing (`s[:n]`). It splits multi-byte characters; the invalid UTF-8 is then rejected by `utf8mb4` columns, which trips the storage-failure counter and disconnects the node. Truncate on rune boundaries (see `safeReportedApplyError` for the existing precedent).
- Do not sanitize only one write path for a node-reported column. `content_hash` is written by both the hello and the ack path; sanitizing one still lets dirty values through the other.
- Do not ignore `loadNode`'s ok flag when reloading a node after a write, and do not answer a request with extra path segments as though they were absent. Both silently produce a wrong response instead of a `404`.
