...
> **⚠️ 2026-08-18 部署策略变更(用户裁决):`C4`(`08-18-controlproto-contract-fixes`)改为 panel/node 同步升级,不做新旧混跑兼容。**
>
> 对 C4 而言,上一段的"滚动升级期间新旧版本混跑不破坏"以及下方 X4 的"提供新panel+旧node、旧panel+新node 两个方向的测试或论证"、第 62 条的"滚动升级顺序"**均已作废**,替代约定见该子任务 prd 的「部署与协议不匹配语义」一节。
>
> 已完成归档的 C1/C2/C3 不受影响——它们本就只动单侧、无 wire 变更,兼容性已按原红线验收通过。
>
> 保持有效的红线:**禁止任何数据库迁移/schema 变更**(这是回滚能力的基础,与升级方式无关),以及各子任务对恶意/故障对端的防护要求(与版本兼容无关)。
